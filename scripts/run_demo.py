...
def run_images(args):
  ...
  # get all image files in input_dir that starts with left
  left_files = glob.glob(f'{args.input_dir}/left*.png')
  left_files.sort()

  for i, left_file in enumerate(tqdm(left_files)):
    right_file = left_file.replace('left', 'right')
    file_basename = os.path.basename(left_file)
    file_basename = file_basename.replace('.png', '')
    output_dir = os.path.join(args.out_dir, file_basename)
    ...
    logging.info(f"PCL saved to {output_dir}")
...
if __name__=="__main__":
  ...
  parser.add_argument('--input_dir', type=str, help='the directory to read images from')
